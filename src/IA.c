...
t_retour calcule_ia(int nbj_max, int* joueur_en_jeux, int taille_plateau, t_case (*plateau)[taille_plateau], t_liste* liste_piece, int cpt, int joueur, int nb_tour_joueur){
     t_retour ret,ret2;
    ...
    for(k = 0; k < nb_case_disponible; k++){ //Pour toutes les cases possibiliter
        ...
        while(!hors_liste(&liste_piece[joueur]) && nb_simulation_piece < 1){//on verifie pour chaque pieces
           if(joueur %2 == 0){
            ...
            while(compte < 8 && nb_simulation_rotation < 2){ //effectue tout les rotation de la piece
                compte ++;
                rotation(piece.mat,1);
                if(compte % 4 == 0){
                    miroir_vertical(piece.mat);
